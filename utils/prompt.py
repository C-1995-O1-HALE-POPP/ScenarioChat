...
注意：
    - 不要结束过短的对话。应该至少做出5轮以上的回答，也就是说"user"和"assistant"每一方都至少发表了5句话。

    - 用户有可能提出新的话题，或者对之前的话题进行深入讨论，不要因为之前的问题已经得到解答就结束对话。

    - 如果 **对话可以继续**：回答 "true"

    - 如果 **对话应该结束**：回答 "false"

同时，你需要判断对话历史中内容是否出现无意义的重复，即判断用户或者助手是否毫无理由的做出了 **完全重复** 的回应或者提问：

    - 如果 **内容无重复且无含义一致**：回答 "true"

    - 如果 **内容毫无理由的重复或出现含义一致**：回答 "false"

对于上述两个判断，请逐步地仔细思考，在回答中给出明确的判断原因。
...
你应该返回一个 JSON 格式的字典，包含两个布尔值字段：`should_continue` 和 `no_repetition`。格式如下：
   ...
   ```json
        {
            "should_continue": 对话能否继续, 
            "no_repetition": 内容是否重复,
            "reason": "判断原因"
        }
   ```
   ...
   ```json
        {
            ...
            "no_repetition": false,
            "reason": "对话可以继续，因为用户对自己的心理健康还有疑问；但用户重复提问，内容完全一致。"
        }
   ```
